...
def run_exp(
    exp_name, gpu_name, kernel_size, depthwise, gap, residual_connection, dropout
):
    ...
    for data_tag in data_tags:
        for i in range(trials):
            ...
            trainer.fit(
                model=module,
                train_dataloader=dmodule.dataloaders[data_tag]["train"],
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--g", required=True, dest="gap")
    ...
    for gap in [False]:
        for residual_connection in [True]:
            for dropout in ["nodrop"]:
                exp_name = f"nrf_logs/bottleneck_{kernel_size}_{depthwise}_{gap}_{residual_connection}_{dropout}"
                run_exp(exp_name, n_gpu, kernel_size, depthwise, gap, residual_connection, dropout)