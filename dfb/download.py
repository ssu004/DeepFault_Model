...
def download_data(root: str, data: str, **kwargs) -> pd.DataFrame:
    if data not in ["cwru", "paderborn", "mfpt", "ottawa"]:
        raise ValueError("data argument must be [cwru | paderborn | mfpt | ottawa]")

    if data == "cwru":
        return download_cwru(root=root, **kwargs)
    if data == "paderborn":
        return download_paderborn(root=root, sample=kwargs["sample"])
    if data == "mfpt":
        return download_mfpt(root=root)
    if data == "ottawa":
        return download_ottawa(root=root)
...
def download_ottawa(root: str) -> pd.DataFrame:
    ...
    if not os.path.isdir(f"{root}"):
        raise ValueError("일단 받아놓은 데이터만 처리합니다")
